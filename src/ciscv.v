...
// CISC-V adds:
//                                     rd     op
// li.w     0000 000 00000 00000 010 rrrrr 1110111  (load 32 bit value)
// li.d     0000 000 00000 00000 011 rrrrr 1110111  (load 64 bit value)
//
//                    rs2   rs1       rd
...
// lh.pre   iiii iiii iiii rrrrr 001 rrrrr 0001011
...
// lw.post  iiii iiii iiii rrrrr 010 rrrrr 0001111
...
reg [31:0] temp;
//wire writeback;
//assign writeback = ~instruction[2];
//reg writeback;
          ...
          case (op)
            ...
            7'b1110111:
              begin
                case (instruction[31:30])
                  2'b01:
                    begin
                      // CISC-V: lx.base rd, rs1, rs2
                      ea <= registers[rs1] + registers[rs2];
                      mem_bus_enable <= 1;
                      mem_write_enable <= 0;
                      mem_address <= registers[rs1] + registers[rs2];
                      state <= STATE_FETCH_LOAD_1;
                    end
                  2'b10:
                    begin
                      // CISC-V: sx.base rd, rs1, rs2
                      ea <= registers[rs1] + registers[rs2];
                      mem_address <= registers[rs1] + registers[rs2];
                      mem_bus_enable <= 0;
                      state <= STATE_STORE_0;
            ...
            7'b0001011:
              begin
                // Load.pre CISC-V.
                ea <= registers[rs1] + sign12(instruction[31:20]);
                registers[rs1] <= registers[rs1] + sign12(instruction[31:20]);
                mem_bus_enable <= 1;
                mem_write_enable <= 0;
                mem_address <= registers[rs1] + sign12(instruction[31:20]);
                state <= STATE_FETCH_LOAD_1;
            ...
            7'b0101011:
              begin
                // Store.pre CISC-V.
                ea <= registers[rs1] + sign12( { funct7, instruction[11:7] } );
                registers[rs1] <= registers[rs1] + sign12( { funct7, instruction[11:7] } );
                mem_address <= registers[rs1] + sign12( { funct7, instruction[11:7] } );
                mem_bus_enable <= 0;
                state <= STATE_STORE_0;
              end
